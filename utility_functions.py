...
def plot_bar_graph(xlist, ylist, title="Bar graph", xaxis_title="", yaxis_title=""):
    """Helper function to plot a seaborn bar graph
...
def plotly_bar_graph(xlist, ylist, title="Bar graph", xaxis_title="", yaxis_title=""):
...
def create_summary_plots(dataframes, titles, rows, cols, super_title, size=(15, 30)):
    ...
    Args:
        ...
        size (tuple, optional): The size of the figure. Defaults to (15,30).
    """
    ...
    for i in range(0, rows):
        for j in range(0, cols):
            df = dataframes.pop(0)
            title = titles.pop(0)
            sns.barplot(ax=axes[i, j], x=df.iloc[:, 0].apply(lambda x: x if len(x) < 25 else x[:25] + '...'),
                        y=df.iloc[:, 1])
            axes[i, j].set_xlabel("")
            axes[i, j].set_ylabel("")
            axes[i, j].set_title(title)
            axes[i, j].tick_params('x', labelrotation=90)
            sns.despine(right=True, top=True)
...
def create_summary_plots_plotly(dataframes, titles, rows, cols, super_title, height=1300):
    """Create a plot of all summaries
...
def compare_churner_nonchurners(stat_df, title, users_marked_df):
    ...
    sns.boxplot(x=joined.iloc[:, 1], y=joined.iloc[:, 2])
    sns.swarmplot(x=joined.iloc[:, 1], y=joined.iloc[:, 2], color=".25")
    plt.show()


def compare_churner_nonchurners_plotly(stat_df, title, users_marked_df):
    """Creates a plot to compare a stat between churners and nonchurners
